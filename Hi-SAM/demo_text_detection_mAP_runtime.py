...
if __name__ == '__main__':
    ...
    time_t2 = time.time()
    time_model_load = time_t2 - time_t1
    ...
    time_t3 = time.time()
    time_data_load = time_t3 - time_t2
    ...
    if args.map :
        ...
        f1_score = round(np.mean(f1_scores, axis=0), 4)
        ...
        # print(f"score_thresh: {score_thresh} | mAP: {res_mAP} | mAP_50: {res_mAP50} | mAP_75: {res_mAP75}")
        print(f"mAP: {res_mAP}")
    ...
    time_t4 = time.time()
    time_task = time_t4 - time_t3

    with open(args.results_log, "a") as f:
        f.write(f"{time_model_load};{time_data_load};{time_task}\n")