...
def main(train_datasets_lst, valid_datasets_lst, args):
    ...
    if not args.eval:
        ...
        train_dataloaders, train_datasets = create_dataloaders(
            train_im_gt_list,
            my_transforms=train_transforms2,
...
def mAP_and_CER_Cal(model, dataset_dict, htr_scores, score_thresh = 0.6, fg_points_num = 100, zero_shot = False, para_dataset = "ctw1500"):
    ...
    gt_path = os.path.join(os.path.dirname(img_path), "gt_xml")
        ...
        if masks is not None:
            for mask, score in zip(masks, scores):
                ...
                if pred_box is not None:
                    ...
                    # pred boxes without connected components filtering
...
def train(args, model, optimizer, train_dataloaders, train_datasets_names, valid_dataloaders, valid_datasets_names, valid_datasets, valid_dataloaders2):
    ...
    htr_scores = HTRScores(
        args.device,
        f"{args.data_root}/{args.htr_charset}",
        f"{args.data_root}/val/Images",
        f"{args.data_root}/val/gt_xml",
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        # if you want to skip LR drop for some reason, set args.LR_drop True and sets the args.lr to a small value like 1e-5
        ...
        if (epoch - epoch_start) % args.valid_period == 0 or (epoch + 1) == epoch_num:
            ...
            ### CAL mAP #################
            mAP_result, cer_result = mAP_and_CER_Cal(model, valid_datasets, htr_scores)
            if misc.is_main_process():
                ...
                print(f"cer_result: {cer_result}")
                if cer_result < best_CER:
                    checkpoint = {
                        'model': model.module.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'epoch': epoch
                    }
                    torch.save(checkpoint, os.path.join(args.output, ds_name+"_best_CER.pth"))
                    best_CER = cer_result
                    val_stable_count = 0
                    best_epoch = epoch
                else:
        ...
        if (val_stable_count >= args.early_stop_patience) and LR_drop :
            ...
            torch.save(checkpoint, args.output + model_name)
            if args.wandb and is_rank0():
...
if __name__ == "__main__":
    ...
    read2016_train = {
        "name": "READ_2016",
        "im_dir": f"{args.data_root}/train/Images",
        "gt_dir": f"{args.data_root}/train/bin_masks_otsu_hi-sam",
        "im_ext": f".{args.img_ext}",
        "gt_ext": ".png",
    }
    totaltext_train = {
    ...
    read2016_val = {
        "name": "READ_2016",
        "im_dir": f"{args.data_root}/val/Images",
        "gt_dir": f"{args.data_root}/val/bin_masks_otsu_hi-sam",
        "im_ext": f".{args.img_ext}",
        "gt_ext": ".png",
    }
    read2016_test = {
        "name": "READ_2016",
        "im_dir": f"{args.data_root}/test/Images",
        "gt_dir": f"{args.data_root}/test/bin_masks_otsu_hi-sam",
        "im_ext": f".{args.img_ext}",
        "gt_ext": ".png",
    }
    totaltext_test = {
