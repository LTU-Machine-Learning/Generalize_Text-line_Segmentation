...
from utils.export_for_inference import export_hisam_for_inference
import glob
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(args.output, "lines"), exist_ok=True)
    ...
    for path in tqdm(args.input):
        ...
        if masks is not None:
            for mask, score in zip(masks, scores):
                ...
                if pred_box is not None:
                    ...
                    ########### Extract the Image Lines and Save ##########
                    # x_min, y_min, x_max, y_max = pred_box
                    # crop_image = image[y_min:y_max, x_min:x_max]
                    # blank_image = np.ones((y_max - y_min, x_max - x_min, 3), dtype=np.uint8) * 255
                    # crop_mask = mask[y_min:y_max, x_min:x_max]
                    # blank_image[crop_mask == 1] = crop_image[crop_mask == 1]
                    # masked_image_line = blank_image

                    # cv2.imwrite(f"{args.output}/lines/{img_id}_{idx:02}.png", cv2.cvtColor(crop_image, cv2.COLOR_RGB2BGR))
                    # cv2.imwrite(f"{args.output}/{img_id}_{idx:02}.png", cv2.cvtColor(masked_image_line, cv2.COLOR_RGB2BGR))
                    idx += 1
        ...
        if args.map :
            ...
            f1_scores.append(compute_f1(gt_boxes, pred_boxes)["f1"])
    ...
    if args.map :
        ...
        with open(args.results_log, "a") as f:
            f.write(f"{args.exp_id};{score_thresh};{nms_thresh0};{res_mAP};{f1_score};{cer};{modelname};{args.text}\n")
