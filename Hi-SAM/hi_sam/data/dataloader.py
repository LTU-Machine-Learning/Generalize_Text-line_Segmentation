...
class FlipLR(object):
    ...
    def __call__(self, sample: dict) -> dict:
        ...
        if random.random() < self.prob:  # flip with probability `prob`
            sample['image'] = cv2.flip(image, 1)   # Horizontal flip
            sample['label'] = cv2.flip(label, 1)
            sample['line_masks'] = cv2.flip(masks, 1)
...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name == "READ_2016":
            xml_path = (Path(im_path).with_suffix(".xml").as_posix().replace("Images", "gt_xml"))
