...
from utils.HTR_CRNN.evaluate.metrics.evaluation_recognition import nb_chars_from_list, nb_words_from_list, edit_wer_from_list
from utils.HTR_CRNN.evaluate.metrics.metrics_counter import MetricLossCERWER
...
def evaluate_one_epoch_crnn(data_loader,
    ...
    with torch.no_grad():
        for index_batch, batch_data in enumerate(data_loader):
            ...
            metrics_main.add_wer(edit_wer_from_list(y_gt_txt, predictions_text, use_tokenizer_wer),
                                 nb_words_from_list(y_gt_txt, use_tokenizer_wer))
            ...
            top_aux = [torch.argmax(lp, dim=1).detach().cpu().numpy()[:x_reduced_len[j]] for j, lp in enumerate(output_log)]
            ...
            metrics_shortcut.add_wer(edit_wer_from_list(y_gt_txt, predictions_text, use_tokenizer_wer),
                                     nb_words_from_list(y_gt_txt, use_tokenizer_wer))
            ...
            if index_batch == 0:
                ...
                for i in range(nb_pred_to_print):
                    ...
                    if print_frames:
                        char_sequence = convert_int_to_chars(top[i], char_list)
                        char_sequence = char_sequence.replace("<BLANK>", "-")
                        print(char_sequence)
                    print("-----Predictions shortcut:-----")
                    print(predictions_text_aux[i])
                    if print_frames:
                        char_sequence = convert_int_to_chars(top_aux[i], char_list)
                        char_sequence = char_sequence.replace("<BLANK>", "-")
                        print(char_sequence)
