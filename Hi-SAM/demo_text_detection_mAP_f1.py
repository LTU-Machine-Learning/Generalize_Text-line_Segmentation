...
if __name__ == '__main__':
    ...
    gt_path = os.path.join(os.path.dirname(args.input[0]), "YOLO_boxes") # gt
    ...
    metric = MeanAveragePrecision(iou_type="bbox")
    f1_scores_50, f1_scores_75, f1_scores_80, f1_scores_85, f1_scores_90, f1_scores_95 = [], [], [], [], [], []
    for path in tqdm(args.input):
        ...
        if args.map :
            ...
            f1_scores_50.append(compute_f1(gt_boxes, pred_boxes, iou_thresh=0.5, page_size=(img_w, img_h) )["f1"])
            f1_scores_75.append(compute_f1(gt_boxes, pred_boxes, iou_thresh=0.75, page_size=(img_w, img_h) )["f1"])
            f1_scores_80.append(compute_f1(gt_boxes, pred_boxes, iou_thresh=0.8, page_size=(img_w, img_h) )["f1"])
            f1_scores_85.append(compute_f1(gt_boxes, pred_boxes, iou_thresh=0.85, page_size=(img_w, img_h) )["f1"])
            f1_scores_90.append(compute_f1(gt_boxes, pred_boxes, iou_thresh=0.9, page_size=(img_w, img_h) )["f1"])
            f1_scores_95.append(compute_f1(gt_boxes, pred_boxes, iou_thresh=0.95, page_size=(img_w, img_h) )["f1"])
    ...
    if args.map :
        ...
        print(f"F1 Score_50: {round(np.mean(f1_scores_50, axis=0), 4)}")
        print(f"F1 Score_75: {round(np.mean(f1_scores_75, axis=0), 4)}")
        print(f"F1 Score_80: {round(np.mean(f1_scores_80, axis=0), 4)}")
        print(f"F1 Score_85: {round(np.mean(f1_scores_85, axis=0), 4)}")
        print(f"F1 Score_90: {round(np.mean(f1_scores_90, axis=0), 4)}")
        print(f"F1 Score_95: {round(np.mean(f1_scores_95, axis=0), 4)}")
        ...
        print(f"mAP_50: {res_mAP50} | mAP_75: {res_mAP75} | mAP: {res_mAP}")
        # print(f"mAP: {res_mAP}")
        ...
        #     f.write(f"{args.exp_id};{score_thresh};{res_mAP};{f1_score};{modelname};{args.text}\n")
