...
from hi_sam.data.dataloader import get_im_gt_name_dict, create_dataloaders, train_transforms, train_transforms2, train_transforms3, eval_transforms, custom_collate_fn
...
def main(train_datasets_lst, valid_datasets_lst, args):
    ...
    if not args.eval:
        ...
        train(args, model, optimizer, train_dataloaders, train_datasets_names, lr_scheduler, valid_dataloaders, valid_datasets_names, valid_datasets_lst)
    else:
...
def mAP_Cal(model, dataset_dict, score_thresh = 0.6, fg_points_num = 100, zero_shot = False, para_dataset = "ctw1500"):
    ...
    metric = MeanAveragePrecision(iou_type="bbox", dist_sync_on_step=False)
    print("GAYAN")
    print(dataset_dict)
    print("GAYAN###################")
    ...
    for path in tqdm(img_root_list):
        ...
        targets = [{
            ...
            "labels": torch.tensor([0] * len(gt_boxes), dtype=torch.int64)
        }]
        ...
        print("####$$$$$$$$$$$$$$$$$$$$$$$")
        if len(pred_boxes) == 0:
            print("CASE: 000000000000000")
            preds = [{
                "boxes": torch.empty((0, 4), dtype=torch.float32),   # empty tensor
                "scores": torch.empty((0,), dtype=torch.float32),
                "labels": torch.empty((0,), dtype=torch.int64)
            }]
        else:
            print("CASE: ACTUAL_ACTIAL")
            preds = [{
                "boxes": torch.tensor(pred_boxes, dtype=torch.float32),
                "scores": torch.tensor(pred_scores, dtype=torch.float32),  # Here the score is actually a confident score predicted by Hi-SAM
                "labels": torch.tensor([0] * len(pred_boxes), dtype=torch.int64)
            }]
...
def train(args, model, optimizer, train_dataloaders, train_datasets_names, lr_scheduler, valid_dataloaders, valid_datasets_names, valid_datasets):
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        if val_stable_count >= args.early_stop:
