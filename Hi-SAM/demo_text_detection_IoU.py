...
def get_Page_IoU_Score(gt_path, img_id, masks):
    ...
    for mask in masks:
        ...
        pred_boxes_4iou.append(pred_box)   

    ious = box_iou(torch.tensor(pred_boxes_4iou), torch.tensor(gt_boxes))   

    # Convert IoU to cost (Hungarian solves min-cost)
    cost_matrix = 1 - ious.numpy()
    # Hungarian algorithm
    row_ind, col_ind = linear_sum_assignment(cost_matrix)
    matched_ious = ious[row_ind, col_ind]    

    matched_ious = matched_ious.numpy()
    matched_ious = np.pad(matched_ious, (0, len(gt_boxes) - matched_ious.shape[0]), 'constant')
...
if __name__ == '__main__':
    ...
    gt_path = os.path.join(os.path.dirname(args.input[0]), "gt")
    ...
    conf_scores = [0.6, 0.65, 0.7, 0.75, 0.8]
    ...
    masks4all_conf_scores = {}
    for score_thresh in conf_scores: 
        masks_page = []
        all_ious = []
        for path in tqdm(args.input):
            ...
            # Calculate IoU for the page
            matched_ious = get_Page_IoU_Score(gt_path, img_id, masks)
            all_ious.extend(matched_ious.tolist())   

        avg_iou = sum(all_ious) / len(all_ious)
        print(f"@ {score_thresh}: {avg_iou}")            
